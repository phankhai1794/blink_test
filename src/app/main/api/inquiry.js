...
export async function editComment(id, content) {
  const response = await axios.patch(`${process.env.REACT_APP_API}/inquiry/comment/${id}`, {
    content
  });
  return response.data;
}
...
export async function changeStatus(id, state) {
  const response = await axios.patch(`${process.env.REACT_APP_API}/inquiry/status/${id}`, {
    state
  });
  return response.data;
}
